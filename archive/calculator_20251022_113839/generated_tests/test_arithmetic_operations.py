...
def test_negate_positive_number():
    assert negate(5.0) == -5.0

def test_negate_negative_number():
    assert negate(-5.0) == 5.0

def test_negate_zero():
    assert negate(0.0) == 0.0

def test_negate_float_precision():
    assert negate(0.1) == -0.1
...
def test_percentage_conversion_float_precision():
    assert percentage_conversion(0.1) == pytest.approx(0.001)
